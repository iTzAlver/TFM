...
class Tree:
    def __init__(self, payload='', id=-1, ntree=-1, day=-1, pt=-1):
        self.nTree = ntree
...
def fbbcm(sentences_vector, threshold, day, correlation_model='bert', showup=False):
    ...
    for _item in range(len(sentences_vector)):
        trees.append(Tree(sentences_vector[_item], _item, _item, day))
    ...
    while status == 'continue':
        con_mtx = _genconmatv2(trees, threshold, correlation_model=correlation_model)
        ...
        for _row in range(len(con_mtx)):
            for _col in range(len(con_mtx[_row])):
                if con_mtx[_row][_col] >= threshold and con_mtx[_col][_row] >= threshold:
                    trees = merge_trees(trees, [trees[_row].nTree, trees[_col].nTree])
                    con_mtx[_row][_col] = -1
                    con_mtx[_col][_row] = -1
    ...
    ticktock = time.time() - tick
    print('Total time in while(continue): ' + str(round(ticktock, 2)))
    print('Time in ' + str(it) + ' iterations.')

    for a_tree in trees:
        a_tree.pt = corr_p(a_tree)
...
def _genconmat(trees, threshold, correlation_model='bert'):
    global model
    global model_use
    sentences_vector = []
    for _item in range(len(trees)):
...
def _genconmatv2(trees, threshold, correlation_model='bert'):
    global model
    global model_use
    sentences_vector = []
    code = []
...
def merge_trees(trees, tree_ids):
    new_trees = []
    ids = tree_ids
    ids.sort()
    target_id = ids[0]
...
def corr_p(tree, mode='prime'):
    rx = _correlate(split_payload(tree.payload))
    T = len(rx)
    if mode == 'prime':
...
def _correlate(sentences_vector):
    global model
    code = model.encode(sentences_vector)
    mtx = []
